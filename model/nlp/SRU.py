...
# reference https://github.com/taolei87/sru/blob/master/cuda_functional.py
...
class SRU_Compute(Function):
    ...
    def forward(self, u, x, bias, init=None, mask_h=None):
        ...
        length = x.size(0) if x.length() == 3 else 1
        batch = x.size(-2)
        d = self.d_out
        k = u.size(-1) // d
        ...
        size = (length, batch, d*bidir) if x.length() == 3 else (batch, d * bidir)
        c = x.new(*size)
        h = x.new(*size)
...
class SRUCell(nn.Module):
    ...
    def forward(self, input, c0=None):
        assert input.length() == 2 or input.length() == 3
...
class SRU(nn.Module):
    ...
    def forward(self, input, c0=None, return_hidden=True):
        assert input.length() == 3 # (len, batch, n_in)
        ...
        if c0 is None:
            ...
            assert c0.length() == 3    # (depth, batch, n_out*dir_)
